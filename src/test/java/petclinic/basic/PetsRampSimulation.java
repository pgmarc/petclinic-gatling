...
public class PetsRampSimulation extends Simulation {
    ...
    ScenarioBuilder rampBasicOwners = scenario("Basic Owners Ramp")
            .feed(csv("basic/owners-ramp.csv"))
            .exec(register, login, petListing, registerPet);

    {
        setUp(
                rampBasicOwners.injectOpen(rampUsers(rampUsers).during(rampDuration)))
